...
class CardEngine:
    def __init__(self, cards: list[Card], strategy: CardStrategy, interface: CardInterface):
    ...
    def run(self):
        ...
        while True:
            ...
            user_input = self.interface.get_user_input("(Press Enter to see the back, or 'q' to quit): ").strip()
            if user_input.lower() == 'q':
                self.interface.notify("Exiting the session.")
                break

            self.interface.display_card_back(card)
            feedback = self.interface.get_user_input("(Press Enter to continue, or 'q' to quit): ").strip()
            if feedback.lower() == 'q':
                self.interface.notify("Exiting the session.")
                break
